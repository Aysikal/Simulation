...
class Earth_and_sun:
    ...
    def update_position(self, objects, delta_t):
        ...
        acc_x = total_fx / self.mass
        acc_y = total_fy / self.mass
        
        self.x_vel += rk4_step(self.x_vel, lambda v: acc_x, delta_t)
        self.y_vel += rk4_step(self.y_vel, lambda v: acc_y, delta_t)
        self.x += rk4_step(self.x, lambda x: self.x_vel, delta_t)
        self.y += rk4_step(self.y, lambda y: self.y_vel, delta_t)
...
def main():
    ...
    writer = imageio.get_writer('test,1000,100,RK4.mp4', fps=20)
